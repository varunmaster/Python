...
anotherMess = "kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT\
...
# clue: One small letter, surrounded by EXACTLY three big bodyguards on each of its sides.

answer = list()

# this is using slicing. which means to start at the 3rd index until the end
for l in range(4,len(anotherMess)):
    if ((anotherMess[l - 4]).islower() and (anotherMess[l - 3]).isupper() and ((anotherMess[l - 2])).isupper() and ((anotherMess[l - 1])).isupper() and anotherMess[l].islower() and ((anotherMess[l + 1])).isupper() and ((anotherMess[l + 2])).isupper() and ((anotherMess[l + 3])).isupper() and ((anotherMess[l + 4])).islower()):
        answer += anotherMess[l]
        print anotherMess[l]

print ''.join(answer)

# had to change the algorithm to match the following: xXXXxXXXx
# using XXXxXXX does not work because ??? i dont know why that would not work if it iterates over every
# letter in anotherMess